...
当前版本：**0.3.2**。相比 v0.3.1，这一版补充了 yaml-cpp YAML 后端以覆盖 Windows / clang-cl 等 libfyaml 包不可用的环境，并保留 libfyaml 后端；Windows CI 改为使用 `--enable_yamlcpp`。同时，YAML metadata 的静态检查更严格，通用类型 traits 也收敛到公共 `global/traits.hpp`，供 serialization、ArgParser 等模块共享。
...
**为什么需要这个库**：开发者应将精力聚焦于业务逻辑本身，而非繁琐、易错的协议构造和维护。本库致力于提供一个稳定、可测试的基础设施，降低协议/数据处理的复杂度。
...
*   **序列化后端**：
    ...
    *   YAML: [libfyaml](https://github.com/pantoniou/libfyaml) 或 [yaml-cpp](https://github.com/jbeder/yaml-cpp) (可选，按需包含)。同时开启时，默认 `YamlSerializer` 会优先选择 libfyaml；也可以直接使用具体后端类型。
    *   TOML: [toml++](https://github.com/marzer/tomlplusplus) (可选，按需包含)
...
add_requires("neko-proto-tools", {
    configs = {
         ...
         enable_libfyaml = false,
         enable_yamlcpp = false,
         enable_tomlplusplus = false
    }
})
...
*   **内置序列化器**：
    ...
    *   `YamlSerializer`: 使用 libfyaml 或 yaml-cpp 提供 YAML 序列化和反序列化支持。
    *   `TomlSerializer`: 使用 toml++ 提供 TOML 序列化和反序列化支持。
*   **选择序列化器**：
...
序列化后端会引入统一的 Parser 集合，常用 STL 容器和基础类型可直接使用，无需再包含单独的 `serialization/types/*.hpp`。当前 JSON、Binary、XML、YAML、TOML 和 schema 生成会尽量复用同一套类型规则，只有格式确实无法表达的语义才由对应后端忽略或特化处理。
...
自定义 C++ 类型应特化 `CustomParser<T>`。读写逻辑可复用已有 parser 入口，因此同一扩展可以用于 JSON、Binary、XML、YAML 和 TOML 等后端。`CustomParser<T>` 优先于内建反射和 STL parser。
...
*   [x] RapidJSON、simdjson、Binary、XML、YAML、TOML、schema 统一到通用 Parser 层
*   [x] 使用 pugixml 实现 XML Reader/Writer 后端
*   [x] 使用 libfyaml / yaml-cpp 实现 YAML Reader/Writer 后端
*   [x] 使用 toml++ 实现 TOML Reader/Writer 后端
...
*   [x] 为 `NekoRpcBackend` 补充 hello 协商、预算内 method table 下发和 method-id 错误恢复，设计见 [`docs/rpc_method_id_design.md`](docs/rpc_method_id_design.md)。
...
*   [ ] 继续优化内部代码结构，收敛长流程和重复实现，修复后续发现的问题。
*   [ ] 优化使用体验、错误诊断和文档示例，降低新项目接入成本。
*   [ ] 增加压力测试和性能评估测试，并与其他同类库做基准对比。

---

## 9. 开发历史 (部分里程碑)
*   **v0.3.2**
    *   新增 yaml-cpp YAML Reader/Writer 后端；`YamlSerializer` 在同时启用 libfyaml 和 yaml-cpp 时仍优先选择 libfyaml，也可以直接使用具体后端类型。
    *   Windows / clang-cl CI 改为启用 `enable_yamlcpp`，避免 libfyaml xmake 包在 Windows 平台不可用导致构建失败。
    *   补强 YAML metadata 静态检查：`yaml_tag` 支持 `!` 开头的 local tag 和 URI 形式的 global tag，禁止空白/控制字符；`yaml_anchor` 检查空白和 YAML anchor 不允许的分隔字符。
    *   `yaml_scalar_style_tag` / `yaml_collection_style_tag` 会在带 tag 的读写路径上进行编译期类型检查，避免把 scalar style 用到容器或把 collection style 用到标量字段。
    *   将 string-like、optional-like 解包、已知集合、scalar-like、collection-like 等通用类型 traits 提到 `global/traits.hpp`，serialization 与 ArgParser 共用同一套命名和规则。
    *   带 tag 的 serializer 字段进入 Parser 读写分发时会触发 tag 的 `constexpr_check`，让 tag 约束在实际使用点尽早暴露。

*   **v0.3.1**
    *   拆分序列化 parser 分发：写入走 `WriteParser<Writer, T>`，读取走 `ReadParser<Reader, T>`，schema 走 `SchemaParser<T>`；用户扩展点改为公开的 `CustomParser<T>`。
    *   新增 YAML / TOML 序列化后端：`YamlSerializer` 支持 libfyaml，`TomlSerializer` 基于 toml++，并接入统一 Parser 类型规则。
    ...
    *   完善 `NekoRpcBackend` 连接状态模型：补充 hello 协商、动态 method-id 表刷新、delta/signature/兼容错误、预算内错误响应附表和客户端最多一次恢复重试。
    *   增加 `NekoRpcBackend` 压缩 TLV、可替换压缩 codec policy 和基础压缩统计，压缩能力保持在 backend context/session 内部。

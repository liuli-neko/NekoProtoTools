...
当前版本：**0.3.1**。相比 v0.3.0，这一版补齐了反射驱动的 ArgParser、YAML/TOML 序列化后端、统一 tags/反射元数据基础设施，以及 `NekoRpcBackend` 的 hello 协商、动态 method-id 表更新、预算内错误恢复和压缩扩展；序列化侧的 `sa::Result<T>` 在支持 `<expected>` 的环境会直接使用 `std::expected<T, sa::Error>`，否则使用内置兼容实现。
...
*   **序列化后端**：
    ...
    *   YAML: [libfyaml](https://github.com/pantoniou/libfyaml) (可选，按需包含)
...
*   **内置序列化器**：
    ...
    *   `YamlSerializer`: 使用 libfyaml 提供 YAML 序列化和反序列化支持。
...
*   [x] 使用 libfyaml 实现 YAML Reader/Writer 后端
...
*   **v0.3.1**
    ...
    *   新增 YAML / TOML 序列化后端：`YamlSerializer` 基于 libfyaml，`TomlSerializer` 基于 toml++，并接入统一 Parser 类型规则。

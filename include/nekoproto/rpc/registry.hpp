...
inline auto rpcJoinName(std::string_view prefix, std::string_view name) -> std::string {
    if (prefix.empty()) {
...
void rpcVisitMethod(Method& method, std::string_view field_name, const RpcPropertyPatch& field_properties,
    ...
    if (method.rpcNoPrefix()) {
        ...
        method_prefix = rpcJoinName(prefix, method.rpcPrefix());
    } else {
        method_prefix = std::string(prefix);
    }

    const auto full_name = rpcJoinName(method_prefix, method.declaredName());
...
void rpcVisitField(Field& field, std::string_view field_name, const Tags& tags, std::string_view prefix, Fn&& fn) {
    ...
    const auto field_properties = collectRpcProperties(tags);
    if constexpr (RpcMethodObject<FieldT>) {
        rpcVisitMethod(field, field_name, field_properties, prefix, fn);
        ...
        if (field_properties.noPrefix.value_or(false)) {
            ...
            next_prefix = rpcJoinName(prefix, *field_properties.prefix);
        } else if (!field_name.empty()) {
            next_prefix = rpcJoinName(prefix, field_name);
        } else {
            next_prefix = std::string(prefix);
        }
        forEachRpcMethod(field, fn, next_prefix);
    }
}

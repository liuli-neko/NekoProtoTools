...
public:
    ...
    void configure(std::size_t maxActive, std::size_t maxQueued) {
...
public:
    ...
    virtual auto call(const typename Backend::DecodedRequest& request, typename Backend::ResponseValues& responses,
                      const RpcRequestContext* context) -> ilias::Task<void> = 0;
...
public:
    ...
    void configureExecution(std::size_t maxActive, std::size_t maxQueued,
    ...
    void bindRpcMethod(std::string_view name, traits::FunctionT<RetT(Args...)> func,
                       const std::array<std::string_view, N>& arg_names) {
        return registerRpcMethod<RpcMethodDynamic<RetT(Args...)>>(arg_names, name, std::move(func));
    }

    template <typename RetT, typename... Args, std::size_t N>
    void bindRpcMethod(std::string_view name, traits::FunctionT<ilias::IoTask<RetT>(Args...)> func,
                       const std::array<std::string_view, N>& arg_names) {
        registerRpcMethod<RpcMethodDynamic<RetT(Args...)>>(arg_names, name, std::move(func));
    }

    template <typename Method, typename RetT, typename... Args>
    void bindRpcMethodWithContext(Method& metadata,
    ...
    template <typename RetT, typename... Args, std::size_t N>
    void bindRpcMethodWithContext(std::string_view name,
                                  traits::FunctionT<ilias::IoTask<RetT>(const RpcRequestContext&, Args...)> func,
                                  const std::array<std::string_view, N>& argNames) {
        ...
        mHandlers[key] = std::make_unique<RpcContextMethodWrapperImpl<Backend, std::unique_ptr<Metadata>, Function>>(
    ...
    auto connectionStatus(const RpcRequestContext& context) const -> std::map<std::string, std::uint64_t> {
        ...
        for (const auto& [key, state] : mRequestStates) {
            if (key.session != session || stringifyId(key.id) == context.requestId()) {
                continue;
            }
    ...
    auto connectionTasks(const RpcRequestContext& context) const -> std::map<std::string, std::string> {
        ...
        std::scoped_lock lock(mActiveMutex);
        for (const auto& [key, state] : mRequestStates) {
            const auto requestId = stringifyId(key.id);
    ...
    auto processMessage(std::span<const std::byte> data, typename Backend::ServerContext& context,
                        ...
                        const RpcPeerInfo* peer = nullptr) -> ilias::Task<Message> {
        Message buffer;
        ...
        for (auto& slot : responseSlots) {
            responses.insert(responses.end(), std::make_move_iterator(slot.begin()),
...
private:
    auto executeRequest(RpcMethodWrapperBase<Backend>& handler, const typename Backend::DecodedRequest& request,
    ...
    auto runAdmittedRequest(RpcMethodWrapperBase<Backend>& handler, const typename Backend::DecodedRequest& request,
        ...
        if (handler.usesContext()) {
            context.emplace(RpcRequestContextAccess::make(Backend::methodName(request),
                                                          stringifyId(Backend::id(request)), deadline, peer, session));
    ...
    static auto deadlineFromNow(const std::optional<std::chrono::nanoseconds>& timeout)
    ...
    void registerRpcMethod(const std::array<std::string_view, N>& array, std::string_view name,
                           traits::FunctionT<Callable> func) {
    ...
    template <typename T>
    auto handle(const typename Backend::DecodedRequest& request, typename Backend::ResponseValues& responses,
    ...
    auto handleWithContext(const typename Backend::DecodedRequest& request, typename Backend::ResponseValues& responses,
                           T& metadata, Function& function, const RpcRequestContext& context) -> ilias::Task<void> {
        ...
        try {
            result = co_await std::apply(
                [&](auto&&... args) { return function(context, std::forward<decltype(args)>(args)...); },
    ...
    static auto stringifyId(const Value& value) -> std::string {
        ...
        if constexpr (std::is_same_v<Type, std::monostate>) {
            ...
            return std::visit([](const auto& active) { return stringifyId(active); }, value);
        } else {
            return {};
        }
    }
...
auto RpcMethodWrapperImpl<Backend, T>::call(const typename Backend::DecodedRequest& request,
    ...
    return mSelf->handle(request, responses, *mMethodData);
}

template <RpcBackend Backend, typename T, typename ContextFunction>
auto RpcContextMethodWrapperImpl<Backend, T, ContextFunction>::call(const typename Backend::DecodedRequest& request,
                                                                    typename Backend::ResponseValues& responses,
    ...
    co_await mSelf->handleWithContext(request, responses, *mMethodData, mFunction, *context);
}

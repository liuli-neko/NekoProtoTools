...
concept RpcBackend = requires(std::span<const std::byte> message,
    ...
    { Backend::decodeIncoming(message) } -> std::same_as<typename Backend::DecodeResult>;
    ...
    { Backend::encodeResponses(responses, batch) } -> std::same_as<typename Backend::Message>;
    { Backend::clientNotInitError() } -> std::same_as<std::error_code>;
    { Backend::notificationOk() } -> std::same_as<std::error_code>;
};

template <typename Backend, typename T>
concept BackendSerializable = std::is_void_v<T> || requires {
    requires Backend::template serializable<T>();
};

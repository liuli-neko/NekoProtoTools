...
struct Parser<R, W, std::tuple<Ts...>, void> {
        ...
        const auto writeElement = [&]<std::size_t I>() {
            if (result) {
                result = parser_context(
                    parser_write<R, W>(writer, std::get<I>(value), typename parsing::Parent<W>::Array{&array}),
    ...
    static ParserResult readImpl(const typename R::InputArrayType& array, Tuple& value,
                                 std::index_sequence<Is...>) {
        ParserResult result;
        const auto readElement = [&]<std::size_t I>() {
            if (result) {
                result = parser_context(parser_read<R, W>(R::arrayElement(array, I), std::get<I>(value)),
    ...
    static ParserResult read(typename R::InputValueType in, Tuple& value, const Tags& /*tags*/) {
        ...
        if (actualSize != sizeof...(Ts)) {
            return parser_error(sa::ErrorCode::InvalidLength,
                                "Expected tuple with " + std::to_string(sizeof...(Ts)) + " elements, got " +
    ...
    static parsing::schema::Type toSchema() {
        ...
        schema.prefixItems    = {parser_schema<R, W, Ts>()...};
...
template <typename R, typename W, typename K, typename V>
struct Parser<R, W, std::pair<K, V>, void> {
    ...
    static ParserResult write(W& writer, const Pair& value, const ParentType& parent, const Tags& /*tags*/) {
        ...
        auto result =
            parser_write<R, W>(writer, value.first, typename parsing::Parent<W>::Object{"first", &object});
        if (!result) {
            return parser_context(std::move(result), "Failed to write pair field 'first': ");
        }
        return parser_context(
            parser_write<R, W>(writer, value.second, typename parsing::Parent<W>::Object{"second", &object}),
    ...
    static ParserResult read(typename R::InputValueType in, Pair& value, const Tags& /*tags*/) {
        ...
        auto result = parser_read<R, W>(first.value(), value.first);
        if (!result) {
        ...
        return parser_context(parser_read<R, W>(second.value(), value.second),
                              "Failed to parse pair field 'second': ");
    }

    static parsing::schema::Type toSchema() {
        parsing::schema::Type::Object schema;
        schema.properties.emplace("first", parser_schema<R, W, K>());
        schema.properties.emplace("second", parser_schema<R, W, V>());

...
template <typename T>
consteval auto canReflectRefAnyFromRvalueIndex() -> bool {
    return requires { Reflect<T>::value(T{}, 0); };
}

template <typename T>
consteval auto canReflectRefAnyFromRvalueName() -> bool {
...
static_assert(!canReflectRefAnyFromRvalueIndex<Test1>());
static_assert(!canReflectRefAnyFromRvalueName<Test1>());
...
struct Test2 {
    ...
    struct Neko {
        constexpr static auto value = // NOLINT
            Object("member1", makeTags<CustomTags{.tag1 = false, .tag2 = true, .tag3 = false}>(&Test2::member1),
                   "member2",
                   makeTags<JsonTag{.flat = true, .skippable = false, .raw_string = false}>(&Test2::member2),
                   "member3", makeTags<BinaryTag{.fixed_length = sizeof(member3)}>([](auto&& self) -> auto& {
                       return self.member3;
                   }),
...
TEST(Reflection, RefObjectValue) {
    ...
    Reflect<Test2>::forEach(test, [](auto& field, std::string_view name, const auto& tags) {
        ...
        if constexpr (tag_query::has<tag_property::FixedLength<void>>(decltype(tags){})) {
            NEKO_LOG_INFO("test", "binary tags: fixed_length={}",
                          tag_query::get<tag_property::FixedLength<int>>(tags));
        } else if constexpr (tag_query::has<tag_property::Flat<int>>(decltype(tags){}) ||
                             tag_query::has<tag_property::Skippable>(decltype(tags){}) ||
                             tag_query::has<tag_property::RawString>(decltype(tags){})) {
...
consteval auto testNamedForEachMeta() -> bool {
    ...
    auto tuple        = Reflect<Test2>::forEachMeta(
        [&]<typename Field>(std::type_identity<Field>, std::string_view name, const auto& tags) {
            ...
            if constexpr (std::is_same_v<std::decay_t<decltype(tags)>,
                ...
                hasJsonTag = true;
                return 2;
            } else if constexpr (tag_query::has<tag_property::FixedLength<void>>(decltype(tags){})) {
                hasBinaryTag = true;
                return name;
            }
        });
...
static_assert(testNamedForEachMeta());
...
struct Test3 {
    ...
    struct Neko {
        constexpr static auto value = // NOLINT
            Array(makeTags<BinaryTag{.fixed_length = sizeof(prr.member1)}>(
                      [](auto&& self) -> auto& { return self.prr.member1; }),
                  makeTags<JsonTag{.skippable = false, .raw_string = true}>(&Test3::member2));
    };
};
...
TEST(Reflection, RefObjectArray) {
    ...
    Reflect<Test3>::forEach(test, []<typename T>(T& field, const auto& tags) {
        NEKO_LOG_INFO("test", "field: {}", field);
        if constexpr (tag_query::has<tag_property::FixedLength<void>>(decltype(tags){})) {
            NEKO_LOG_INFO("test", "binary tags: fixed_length={}", tag_query::get<tag_property::FixedLength<T>>(tags));
...
consteval auto testPositionalForEachMeta() -> bool {
    std::size_t count = 0;
    bool hasInt       = false;
...
static_assert(testPositionalForEachMeta());
...
struct Meta<::TaggedStatus, void> {
    constexpr static auto value = // NOLINT
        Enumerate("ok", makeTags<comment_tag<"completed">>(::TaggedStatus::Ok), "warning",
                  makeTags<comment_tag<"needs attention">, JsonTag{.skippable = true}>(::TaggedStatus::Warning),
...
struct Meta<::AutoTaggedStatus, void> {
    constexpr static auto value = // NOLINT
        Enumerate(makeTags<comment_tag<"alpha message">>(::AutoTaggedStatus::Alpha), ::AutoTaggedStatus::Beta);
};
} // namespace nekoproto

consteval auto testEnumTagsForEachMeta() -> bool {
    std::size_t count = 0;
    bool sawOk        = false;
...
static_assert(testEnumTagsForEachMeta());
...
struct Test4 {
    ...
    struct Neko {
        constexpr static auto value = makeTags<JsonTag{.flat = true}>(&Test4::member1); // NOLINT
    };
};
...
struct Test5 {
    ...
    struct Neko {
        ...
        constexpr static auto value       = makeTags<35>([](auto&& self) -> auto& { return self.member2; }); // NOLINT
    };
};
...
struct Meta<Test6, void> {
    constexpr static auto value = // NOLINT
        Object(
            ...
            makeTags<JsonTag{.flat = true}>(&Test6::member2));
};
} // namespace nekoproto
...
TEST(Reflection, Local) {
    ...
    Reflect<Test6>::forEach(test, [](auto& field, std::string_view name, const auto& tags) {
        if constexpr (tag_query::hasTag<JsonTag>(decltype(tags){})) {
            NEKO_LOG_INFO("test", "{}: {}, tags={}", name, field, tag_query::getTag<JsonTag>(tags).flat);
        } else {

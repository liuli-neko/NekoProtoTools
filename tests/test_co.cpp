...
struct PromiseBase
{
   ...
   void set_notify_waiter(std::function<void()> notify_waiter) {
      notify_waiter_ = notify_waiter;
   }
   void cancel() {
      cancel_ = 1;
   }
...
struct Task : public TaskBase {
   ...
   Task(const handle_type &h) : handle_(h) { CTXTRACER }
   Task(const handle_type &h, std::string name) : handle_(h) {
      CTXTRACER
...
struct Awaitable <int, Task<T>> {
   ...
   Awaitable(int &&value, const Task<T> &waiter) : value_(value), waiter_(waiter) { CTXTRACER }
   Awaitable(const int &value) : value_(value) { CTXTRACER }
   bool await_ready() {
      CTXTRACER
      return true;
   }
...
struct Awaitable<Task<T>, Task<U>> {
   ...
   Awaitable( Task<T> &&task, const Task<U> &waiter) : task_(task), waiter_(waiter) { CTXTRACER }
   Awaitable(const Task<T> &task, const Task<U> &waiter) : task_(task), waiter_(waiter) { CTXTRACER }
   bool await_ready() {
      CTXTRACER

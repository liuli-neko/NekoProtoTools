...
API 层不应增加 method-id、compression、trace、auth 这类专用参数。扩展专属恢复可以由 backend context 通过统一 hook 执行，不能变成 RPC API 参数或 endpoint 职责。
...
在这条链上，method-id、压缩、trace、auth、retry 都不能让 RPC client 出现扩展专属分支。RPC client 只知道它正在让 backend 处理一条调用，并在 backend 表示可恢复时重放一次。
...
服务端发现 method-id 失效时，只返回 backend error。错误 response 可以在受限预算内携带当前 method table 或版本提示，但服务端不替客户端刷新、重试或回退 name 请求。刷新、重试、降级都是客户端策略。
...
- 在客户端 hello、显式刷新请求或 method-id 错误响应中按预算暴露新状态。
- 什么都不做，直到下一次连接协商。
...
| 响应解码 | 还原调用结果或 backend error，更新 backend 状态，并通过 backend recovery hook 做有上限的安全重放 | 在 RPC client 或 endpoint 中写扩展特例 |
...
- 默认客户端策略可以在 method-id 解码错误后应用错误响应携带的小表，并最多重试当前调用一次。
...
- 没有有效表时发 name。
- 表明确有效时可以发 id。
- 收到 table outdated、id not found、signature mismatch 等错误时，优先应用错误 response 中的 method table TLV。
- 如果错误发生在服务端 method-id 解码阶段且表已刷新，可以最多重放一次当前调用。
- 如果响应没有携带表，客户端再主动刷新或降级 name 请求。
- 不透明重试已经到达服务端业务逻辑的调用。
...
- 服务端只返回错误。
- 客户端根据策略决定是否重放。
- 已经进入服务端用户逻辑的调用，不能默认不透明重试。
- 扩展专属 retry 必须留在 backend context/hook 内，证明不会重复执行业务逻辑，并有明确次数上限。
